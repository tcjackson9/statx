...
for player_name in player_list:
    ...
    try: 
        ...
        if not data or not position:
            print(f"No stats or position data found for {player_name}. Creating empty table.")
            data = []  # Set an empty data list to ensure an empty table is created
    ...
    # If table exists, skip creating the table
    if result:
        print(f"Table for {player_name} already exists. Skipping table creation.")
        continue
    ...
    print(f"Table for {player_name} created successfully.")

    # If there are stats, insert the player data into the newly created table
    if data:
        if position == "QB":
            insert_query = f"""
        ...
        # Commit the transaction after data insertion
        connection.commit()
        print(f"Data for {player_name} inserted successfully into table '{table_name}'.")
    else:
        print(f"No stats to insert for {player_name}. Table '{table_name}' created with no data.")
